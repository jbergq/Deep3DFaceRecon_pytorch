...
def read_data(im_path, lm_path, lm3d_std, to_tensor=True):
    ...
    if to_tensor:
        im = torch.tensor(np.array(im) / 255.0, dtype=torch.float32).permute(2, 0, 1)
        lm = torch.tensor(lm)
    return im, lm
...
def main(rank, opt, name="examples"):
    ...
    im_tensors = torch.tensor(np.stack(im_tensors))
    lm_tensors = torch.tensor(np.stack(lm_tensors))
    ...
    visualizer.display_current_results(visuals, 0, opt.epoch, dataset=name.split(os.path.sep)[-1], 
        save_results=True, count=i, names=img_names, add_image=False)
